...
OMD Design Plus uses BEM-styled CSS so that you can override styles easily. But if
...
- Full import: [omd-design-plus-vite-starter](https://github.com/omd-design-plus/omd-design-plus-vite-starter)
- On demand: [unplugin-omd-design-plus/examples/vite](https://github.com/omd-design-plus/unplugin-omd-design-plus)

### By SCSS variables

`style` is written in SCSS.
You can find SCSS variables in [`packages/style/src/common/var.scss`](https://github.com/omd-design-plus/omd-design-plus/blob/dev/packages/style/src/common/var.scss).
...
In the future, we will write documentation for variables that can be customized for each component. You can also directly checkout the source [var.scss](https://github.com/omd-design-plus/omd-design-plus/blob/dev/packages/style/src/common/var.scss).
...
If your project also uses SCSS, you can directly change OMD Design Plus style variables. Create a new style file, e.g. `styles/element/index.scss`:
...
@forward 'omd-design-plus/style/src/common/var.scss' with (
  $colors: (
    'primary': (
      'base': green,
    ),
...
// @use "omd-design-plus/style/src/index.scss" as *;
```
...
Import `element/index.scss` before scss of omd-design-plus to avoid the problem of sass mixed variables, because we need generate light-x by your custom variables.

:::

Create a `element/index.scss` to combine your variables and variables of omd-design-plus. (If you import them in ts, they will not be combined.)

:::tip

In addition, you should distinguish your scss from the element variable scss.
If they are mixed together, each hot update of `omd-design-plus` needs to compile a large number of scss files, resulting in slow speed.
...
// or use unplugin-omd-design-plus
...
```ts
// webpack.config.ts
// use unplugin-omd-design-plus

import OmdDesignPlus from 'unplugin-omd-design-plus/webpack'
